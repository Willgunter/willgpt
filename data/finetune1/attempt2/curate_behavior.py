...
def apply_prompt(client: OpenAI, model: str, system_prompt: str, example: str) -> str:
    user_prompt = (
        ...
        "PART 2: LONG FAILURE-ANALYSIS RESPONSE (OPTIONAL)\n\n"
        "If and only if the document contains a meaningful cause → effect → failure chain, extract ONE long analysis.\n\n"
        "This long analysis must:\n\n"
        "explicitly state the original design assumptions\n\n"
        "explain what changes under extreme conditions\n\n"
        "describe the physical mechanism that causes failure\n\n"
        "remain qualitative and system-level\n\n"
        "contain NO equations\n\n"
        "Format exactly as:\n\n"
        "LONG_ANALYSIS:\n"
        "<one concise paragraph explaining the failure chain>\n\n"
        "If no such causal chain exists, output:\n\n"
        "LONG_ANALYSIS:\n"
        "NONE\n\n"
